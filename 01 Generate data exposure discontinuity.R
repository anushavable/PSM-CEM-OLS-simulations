...
# generate exposure 
a = NA # creates vector 
a[x1<0.5] = rbinom(length(a[x1<0.5]), 1, 0.5) # when x1 < 0.5, samples from binomial dist with pr(1=0.5)
a[x1>=0.5] = rbinom(length(a[x1>=0.5]), 1, 0.95) # when x1 >=0.5, samples from binomial dist with pr(1=0.95)
mean(a[x1<0.5]) #should be around 0.5
mean(a[x1>=0.5]) # should be around 0.95

# generate outcome
y = NA 
y = 0*a + 110*x1 + 10*x2 + rnorm(iter*obs)
...
exp_discon <- rbind(x1, x2, a, y)
write.csv(exp_discon, file = "sims.csv")